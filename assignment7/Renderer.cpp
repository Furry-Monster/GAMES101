...
void para(Vector3f eye_pos, std::vector<Vector3f> &framebuffer,
  ...
  for (uint32_t j = start; j < end; ++j) {
    for (uint32_t i = 0; i < scene.width; ++i) {
      ...
      for (int k = 0; k < spp; k++) {
        float x =
            (2 * (i + step / 2 + step * (k % width)) / (float)scene.width - 1) *
            imageAspectRatio * scale;
        float y = (1 - 2 * (j + step / 2 + step * (k / height)) /
                           (float)scene.height) *
                  scale;
...
void Renderer::Render(const Scene &scene) {
  ...
  //       for (int k = 0; k < spp; k++) {
  //         float x =
  //             (2 * (i + step / 2 + step * (k % width)) / (float)scene.width -
  //             1) * imageAspectRatio * scale;
  //         float y = (1 - 2 * (j + step / 2 + step * (k / height)) /
  //                            (float)scene.height) *
  //                   scale;
  //         Vector3f dir = normalize(Vector3f(-x, y, 1));
  ...
  for (int i = 0; i < thread_num; i++)
    rays.push_back(std::thread(para, eye_pos, std::ref(framebuffer),
                               std::ref(scene), spp, imageAspectRatio, scale,
                               i * thread_step, (i + 1) * thread_step));
  for (int i = 0; i < thread_num; i++)
    rays[i].join();
