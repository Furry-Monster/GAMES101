...
BVHBuildNode *BVHAccel::recursiveBuild(std::vector<Object *> objects) {
  ...
  if (objects.size() == 1) { // 递归结束条件
    ...
    node->bounds = Union(node->left->bounds,
                         node->right->bounds); // 返回pMin pMax构成大包围盒
    return node;
  } else {
    //
    ...
    case SplitMethod::SAH: {
      ...
      for (int i = 1; i < B; i++) {
        ...
        float cost =
            0.125 +
            (leftshapes.size() * SA + rightshapes.size() * SB) / SN; // 计算花费
      ...
      auto beginning = objects.begin();
      auto middling = objects.begin() + (objects.size() * mincostIndex /
                                         B); // 划分点选为当前最优桶的位置
      auto ending = objects.end();
      auto leftshapes = std::vector<Object *>(beginning, middling); // 数组切分
      auto rightshapes = std::vector<Object *>(middling, ending);
...
Intersection BVHAccel::getIntersection(BVHBuildNode *node,
  ...
  if (node == nullptr ||
      !node->bounds.IntersectP(ray, ray.direction_inv, {0, 0, 0}))
    return intersection;
