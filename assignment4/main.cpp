...
cv::Point2f recursive_bezier(const std::vector<cv::Point2f> &control_points,
  ...
  for (int i = 0; i < control_points.size() - 1; i++)
    new_points.push_back((1 - t) * control_points[i] +
                         t * control_points[i + 1]);
...
void bezier(const std::vector<cv::Point2f> &control_points, cv::Mat &window) {
  ...
  for (double t = 0; t < 1.0; t += 0.001) {
    auto processed = recursive_bezier(control_points, t);
    window.at<cv::Vec3b>(processed.y, processed.x)[1] = 255;
  }
}
