    ...
    for (int i = min_x; i < max_x; i++) {
      for (int j = min_y; j < max_y; j++) {
        ...
        if (depth_buf[index] > zp) {
          ...
          Eigen::Vector3f interpolated_color = interpolate(
              alpha, beta, gamma, t.color[0], t.color[1], t.color[2], 1);
          Eigen::Vector3f interpolated_normal = interpolate(
          ...
          //  buffer, pass the color to the shaders first to get the final
          //  color;
          // Before :
          //   set_pixel({i, j, zp}, t.getColor());
